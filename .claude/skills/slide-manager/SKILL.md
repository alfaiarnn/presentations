...
Plano de edição (s04-sipp.yaml):
  ...
  Edição estrutural — confirma? (s/n)
```
...
```
Excluir slide s05 — Delphos (SlideProductDetail)
   ...
   Essa ação não pode ser desfeita pela skill. Confirma? (s/n)
```
...
Slide s05-delphos removido.
   Ordem atual dos slides em index.astro:
     ...
     5. s06-cta
     6. s07-encerramento
   Assets potencialmente órfãos: delphos-dashboard.png
```
...
**Variações deriváveis:**
- **grid-featured** — 1 card grande (destaque) + 3 menores em grid.
- **grid-compact** — Cards menores, até 6 itens, sem descrição.
...
**Variações deriváveis:**
- **detail-mirrored** — Visual à esquerda, conteúdo à direita.
- **detail-full** — Sem mockup. Conteúdo em 2 colunas de content blocks.
- **detail-metrics** — Coluna direita com KpiCards em vez de mockup.
- **detail-comparison** — Duas colunas "antes/depois".
...
**Variações deriváveis:**
- **context-narrative** — Texto corrido com destaque lateral.
- **context-data** — Problemas acompanhados de números/KPIs.
...
**Variações deriváveis:**
- **closing-cta** — Botões/links de ação: contato, demo, documentação.
- **closing-team** — Grid de avatares/nomes da equipe.
- **closing-next-steps** — Lista de próximos passos com ícones.
...
- Qual o tema/objetivo?
- Quem é a audiência principal?
- Quantos slides aproximadamente? (sugestão: 5-9)
- Tem conteúdo pronto ou vamos construir do zero?

#### 4. ESTRUTURAR
Com base no briefing, propor a narrativa da apresentação como arco de storytelling. O usuário pode ajustar antes de começar a criar.

#### 5. CRIAR EM LOTE
Para cada slide da estrutura aprovada, executar o fluxo M3 de forma otimizada — gerar todos os YAMLs com placeholders inteligentes de uma vez.

#### 6. REVISAR
Apresentar resumo final com checklist de UX Copy.

#### 7. FINALIZAR
Confirmar que `index.astro` está com todos os slides referenciados na ordem correta.

### Regras do M5

- **Nunca** excluir layouts, componentes, estilos ou config. Apenas conteúdo.
- **Sempre** arquivar antes de limpar. Sem exceção.
- A numeração recomeça do s01 (conteúdo novo, sem conflito).
